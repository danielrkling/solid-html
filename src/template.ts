import { COMMENT_NODE, COMPONENT_NODE, ELEMENT_NODE, INSERT_NODE, TEXT_NODE, ChildNode } from "./parse";
...
function buildTemplate(nodes: ChildNode[]): HTMLTemplateElement | undefined {
    ...
    if (nodes.some((v) => v.type === ELEMENT_NODE)) {
      ...
      template.innerHTML = nodes.map(buildHTML).join("");
      return template;
    }
  }
